...
const Register = ({ onSwitchToLogin }) => {
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/admin/auth/register`, {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/admin/auth/verify-otp`, {
        adminId,
        otp: otpString
      });
  ...
  const handleResendOtp = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/admin/auth/resend-otp`, { adminId });
      setTimer(60);
      setCanResend(false);
      setOtp(['', '', '', '']);
      setError('');
